    ...
    public partial class PropertyWindow : Form
    {
        ...
        private void PropertyWindow_Load(object sender, EventArgs e) {
            ...
            foreach (Group grp in Group.GroupList)
            {
                ...
                if (grp.Permission == Server.tunnelRank)
                {
                    tunnelrank = grp.name;
                }
                if (grp.Permission == Server.chatSpamRank)
                {
                    chatrank = grp.name;
                }
                if (grp.Permission == Server.blockSpamRank)
                {
                    blockrank = grp.name;
                }
                if (grp.Permission == Server.homeRank)
                {
                    homerank = grp.name;
                }
                if (grp.Permission == Server.maintRank)
                {
                    maintrank = grp.name;
                }
            }
                    ...
                    if (line != "" && line[0] != '#') {
                        ...
                        switch (key.ToLower())
                        {
                            ...
                            case "max-guests":
                                ...
                                catch { Server.s.Log("Invalid " + key + ". Using default."); txtGuests.Text = "7"; }
                                break;
                            case "max-maps":
                                try {
        ...
        private void btnSave_Click(object sender, EventArgs e) 
        {
            ...
            try { Application.OpenForms["TextWindow"].Dispose(); }
            catch { }
