    ...
    public sealed class Player
    {
        ...
        void HandleLogin(byte[] message)
        {
            try
            {
                ...
                {
                    if (group.Permission < Server.maintRank)
                    {
                ...
                if (playerTable.Rows.Count == 0) { if (ip != "127.0.0.1" && !Server.devs.Contains(name.ToLower()) && !Server.staff.Contains(name.ToLower())) { checkGuests(); } }
                playerTable.Dispose();
