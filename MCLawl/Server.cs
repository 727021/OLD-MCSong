...
namespace MCSong
{
    public class Server
    {
        ...
        public event LogHandler OnLogAdmin;
        public event LogHandler OnLogOp;
        public event LogHandler OnLogPublic;
        ...
        public struct Sound
        {
            ...
            public static string chat = "chat.wav";
            public static string error = "error.wav";
            public static string op = "op.wav";
            public static string admin = "admin.wav";
            public static string report = "report.wav";
        }
        ...
        public static string ircNick = "MCSong_IRC_Bot";
        ...
        // Maintenance Mode
        public static bool kickLower = true;
        public static LevelPermission maintRank = LevelPermission.Operator;

        // Anti Tunneling
        public static bool antiTunnel = true;
        public static byte maxDepth = 2;
        public static LevelPermission tunnelRank = LevelPermission.Guest;

        // Block Spam Filter
        public static bool blockSpam = false;
        public static LevelPermission blockSpamRank = LevelPermission.Guest;
        public static int blockSpamCount = 13;
        public static int blockSpamSeconds = 5;
        public static string blockSpamCon = "warn";
        ...
        // Home Maps
        public static bool homes = false;
        public static int homeX = 64;
        public static int homeY = 64;
        public static int homeZ = 64;
        ...
        public void Start()
        {
            ...
            if (!Directory.Exists("extra/sound/")) { Directory.CreateDirectory("extra/sound/"); }
            ...
            catch (Exception e)
            {
                ...
                Server.s.Log("MySQL has been turned off for now.");
                //process.Kill();
                goto skipSQL;
            }
        ...
        public static bool upnpSetup()
        {
            try
            {
                ...
                if (Upnp.AddMapping(ar, "TCP", "MCSong") == true)
                    return true;
                else return false;
            }
            catch (Exception ex)
            {
                ErrorLog(ex);
                s.Log("Failed. Make sure your router supports UPnP."); return false;
            }
        }
        ...
        public static void Maintenance()
        {
            if (maintenanceMode)
            {
                Player.GlobalMessage("Server put in &cMaintenance Mode" + DefaultColor + ".");
                s.Log("Server put in Maintenance Mode");
                if (kickLower)
                {
                    Player.players.ForEach(delegate(Player p)
                    {
                        if (p.group.Permission < maintRank && !Server.devs.Contains(name.ToLower()) && !Server.staff.Contains(name.ToLower()))
                        {
                            p.Kick("Maintenance Mode! Come back later!");
                        }
                    });
                    Player.GlobalMessage("All players ranked below " + Group.findPerm(maintRank).color + Group.findPerm(maintRank).name + Server.DefaultColor + " were kicked.");
                    s.Log("All players ranked below " + Group.findPerm(maintRank).name + " were kicked.");
                }
            }
            else if (!maintenanceMode)
            {
                Player.GlobalMessage("Server taken out of &cMaintenance Mode" + DefaultColor + ".");
                s.Log("Server taken out of Maintenance Mode");
            }
        }
