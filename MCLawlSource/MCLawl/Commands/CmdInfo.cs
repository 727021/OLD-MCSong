    ...
    public class CmdInfo : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            else
            {
                ...
                Player.SendMessage(p, "This server's version: " + c.aqua + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString());
                Player.SendMessage(p, "There are currently " + c.teal + Player.players.Count.ToString() + Server.DefaultColor + " players on " + c.teal + Server.levels.Count.ToString() + Server.DefaultColor + " worlds.");
                if (Server.autorestart) Player.SendMessage(p, "This server is scheduled to restart at " + c.aqua + Server.restarttime.ToString("HH:mm:ss"));

                TimeSpan up = DateTime.Now - Server.timeOnline;
                string upTime = "Time online: " + c.teal;
