    ...
    public sealed class Player
    {
        ...
        void HandleLogin(byte[] message)
        {
            try
            {
                ...
                {
                    Kick("You have been omnibanned. Appeal at http://forums.mcsong.comule.com");
                }
                // Whitelist check.
                ...
                if (playerTable.Rows.Count == 0) { if (ip != "127.0.0.1" && !Server.devs.Contains(name) && !Server.staff.Contains(name)) { checkGuests(); } }
                playerTable.Dispose();
        ...
        public void checkGuests()
        {
            ...
            {
                if (p.group.Permission == Group.Find(Server.defaultRank).Permission)
                {
                    guests += 1;
                }
            }
            if (guests >= Server.guests)
            {
        ...
        public void manualChange(ushort x, ushort y, ushort z, byte action, byte type)
        {
            ...
            if (group.Permission <= Server.tunnelRank)
            {
        ...
        public void HandleCommand(string cmd, string message)
        {
            try
            {
                ...
                if (cmd.ToLower() == "facepalm") { SendMessage("727021's bot army just simultaneously facepalm'd at your use of this command."); return; }
                if (cmd.ToLower() == "herp") { SendMessage("derp"); return; }
        ...
        bool CheckBlockSpam() // Block Spam Filter
        {
            if (spamBlockLog.Count >= spamBlockCount)
            {
                ...
                if (spamTimer < spamBlockTimer && group.Permission <= Server.blockSpamRank)
                {
                    switch (Server.blockSpamCon.ToLower())
                    {
                        case "freeze":
                            if (!frozen)
        ...
        bool CheckChatSpam() // Chat Spam Filter
        {
            if (spamChatLog.Count >= spamChatCount)
            {
                ...
                double spamTimer = DateTime.Now.Subtract(oldestTime).TotalSeconds;
                if (spamTimer < spamChatTimer && group.Permission <= Server.chatSpamRank)
                {
                    switch (Server.chatSpamCon.ToLower())
                    {
                        case "mute":
                            ...
                            Server.s.Log(name + " was muted for chat spam (" + spamChatCount + " messages in " + spamTimer + " seconds)");
                            break;
                        case "freeze":
                            if (!frozen)
                                Command.all.Find("freeze").Use(null, " " + name);
                            ...
                            Server.s.Log(name + " was frozen for chat spam (" + spamChatCount + " messages in " + spamTimer + " seconds)");
                            break;
                        case "jail":
                            if (!jailed)
                                Command.all.Find("jail").Use(null, " " + name);
                            ...
                            Server.s.Log(name + " was jailed for chat spam (" + spamChatCount + " messages in " + spamTimer + " seconds)");
                            break;
                        case "warn":
                            Command.all.Find("warn").Use(null, " " + name);
                            ...
                            Server.s.Log(name + " was warned for chat spam (" + spamChatCount + " messages in " + spamTimer + " seconds)");
                            break;
                        case "kick":
                            Kick(name + " was kicked for chat spam");
                            GlobalMessageOps(color + name + Server.DefaultColor + " was muted for kicked spam.");
                            Server.s.Log(name + " was muted for kicked spam (" + spamChatCount + " messages in " + spamTimer + " seconds)");
                            break;
                        case "ban":
                            Command.all.Find("ban").Use(null, " " + name);
                            Kick(name + " was banned for chat spam");
                            GlobalMessageOps(color + name + Server.DefaultColor + " was banned for chat spam.");
                            Server.s.Log(name + " was banned for chat spam (" + spamChatCount + " messages in " + spamTimer + " seconds)");
                            break;
                        default:
                            if (!muted)
                                Command.all.Find("mute").Use(null, " " + name);
